...
class ImageTokenizer(nn.Module):
    def __init__(self,
        ...
        self.latents = torch.randn(shape_latents, generator=None,
                                   device=device, dtype=torch.float16, layout=torch.strided)

        shape_noise = torch.Size([1, 1024])
        self.noise = torch.randn(
            shape_noise, generator=None, device=device, dtype=torch.float16, layout=torch.strided)
    ...
    def decode(self, indices, negative_indices=None, guidance_scale=10, num_inference_steps=20):
        ...
        if negative_indices is not None:
            ...
            negative_image_embeds = self.model.get_codebook_entry(
                negative_indices)
        else:
...
class SeedLlamaTokenizer(LlamaTokenizer):
    ...
    def load_image_tokenizer(self):
        if not hasattr(self, '_image_tokenizer'):
            if self.encoder_url is not None:
                ...
                assert hasattr(self, 'name_or_path') and os.path.exists(
                    self.name_or_path)
    ...
    def encode_image(
        ...
        assert (image_path is None) + (image_pil is None) + \
            (image_torch is None) == 2
...
class SeedLlamaTokenizerOnly(LlamaTokenizer):
    def __init__(self,
                 vocab_file,
                 unk_token="<unk>",
                 bos_token="<s>",
